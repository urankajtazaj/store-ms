...
public class Settings implements Initializable {
    ...
    private void exportDataFunc() {
        ...
        try (Statement st = con.createStatement()) {
            ...
            String view =
                    "CREATE FORCE VIEW PUBLIC.HYRAT_JAVA(FITIMI, FITIMI_LEK) AS SELECT SUM(PRODUKTET.QMIMI_SHITJES - PRODUKTET.QMIMI_STD) AS FITIMI, SUM(PRODUKTET.QMIMI_SHITJES_LEK - PRODUKTET.QMIMI_STD_LEK) AS FITIMI_LEK FROM PUBLIC.PRODUKTET INNER JOIN PUBLIC.SHITJET ON 1=1 WHERE (CAST(SHITJET.KOHA_SHITJES AS DATE) >= DATEADD('week', -1, CURRENT_DATE())) AND ((PRODUKTET.ID = SHITJET.PROD_ID) AND (CAST(SHITJET.KOHA_SHITJES AS DATE) <= CURRENT_DATE()));\n" +
                    "CREATE FORCE VIEW PUBLIC.HYRAT_SOT(FITIMI, FITIMI_LEK) AS SELECT SUM(PRODUKTET.QMIMI_SHITJES - PRODUKTET.QMIMI_STD) AS FITIMI, SUM(PRODUKTET.QMIMI_SHITJES_LEK - PRODUKTET.QMIMI_STD_LEK) AS FITIMI_LEK FROM PUBLIC.PRODUKTET INNER JOIN PUBLIC.SHITJET ON 1=1 WHERE (CAST(SHITJET.KOHA_SHITJES AS DATE) >= DATEADD('day', 0, CURRENT_DATE())) AND ((PRODUKTET.ID = SHITJET.PROD_ID) AND (CAST(SHITJET.KOHA_SHITJES AS DATE) <= CURRENT_DATE()));\n" +
                    "CREATE FORCE VIEW PUBLIC.HYRAT_MUAJ(FITIMI, FITIMI_LEK) AS SELECT SUM(PRODUKTET.QMIMI_SHITJES - PRODUKTET.QMIMI_STD) AS FITIMI, SUM(PRODUKTET.QMIMI_SHITJES_LEK - PRODUKTET.QMIMI_STD_LEK) AS FITIMI_LEK FROM PUBLIC.PRODUKTET INNER JOIN PUBLIC.SHITJET ON 1=1 WHERE (CAST(SHITJET.KOHA_SHITJES AS DATE) >= DATEADD('month', -1, CURRENT_DATE())) AND ((PRODUKTET.ID = SHITJET.PROD_ID) AND (CAST(SHITJET.KOHA_SHITJES AS DATE) <= CURRENT_DATE()));\n" +
                    ...
                    "create force view te_hyrat as select (select sum(vshitjet.total) from vshitjet where datediff('month', cast(vshitjet.koha_shitjes as date), current_date) = 0) as muaji, (select sum(vshitjet.total) from vshitjet where datediff('week', cast(vshitjet.koha_shitjes as date), current_date) = 0) as java, (select sum(vshitjet.total) from vshitjet where datediff('day', cast(vshitjet.koha_shitjes as date), current_date) = 0) as sot from shitjet, produktet where shitjet.prod_id = produktet.id;\n" +
