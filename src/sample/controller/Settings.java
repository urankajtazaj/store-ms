    ...
    private void exportDataFunc() {
        ...
        try (Statement st = con.createStatement()) {
            ...
            st.execute("script to '" + file.getAbsolutePath() + "' table produktet,punetoret,perdoruesi,konsumatoret," +
                    "shitjet,rec,kat_prod,departamenti,priv,pushimet,tvsh,target,shteti,qytetet,valuta");
...
//                    "CREATE FORCE VIEW PUBLIC.HYRAT_SOT(FITIMI) AS SELECT SUM((PRODUKTET.QMIMI_SHITJES - PRODUKTET.QMIMI_STD) * SHITJET.SASIA) AS FITIMI FROM PUBLIC.PRODUKTET INNER JOIN PUBLIC.SHITJET ON 1=1 WHERE (PRODUKTET.ID = SHITJET.PROD_ID) AND (CAST(SHITJET.KOHA_SHITJES AS DATE) = CURRENT_DATE());\n" +
                    "create view hyrat_sot as select sum(produktet.qmimi_shitjes - produktet.qmimi_std) as fitimi from produktet, shitjet where produktet.id = shitjet.prod_id and cast(shitjet.koha_shitjes as date) <= current_date() and cast(shitjet.koha_shitjes as date) >= dateadd('week', -1, current_date());\n" +
                    "create view hyrat_sot as select sum(produktet.qmimi_shitjes - produktet.qmimi_std) as fitimi from produktet, shitjet where produktet.id = shitjet.prod_id and cast(shitjet.koha_shitjes as date) <= current_date() and cast(shitjet.koha_shitjes as date) >= dateadd('day', -1, current_date());\n" +
                    "create view hyrat_muaj as select sum(produktet.qmimi_shitjes - produktet.qmimi_std) as fitimi from produktet, shitjet where produktet.id = shitjet.prod_id and cast(shitjet.koha_shitjes as date) <= current_date() and cast(shitjet.koha_shitjes as date) >= dateadd('month', -1, current_date());\n" +
...
//                    "CREATE FORCE VIEW PUBLIC.TE_HYRAT(MUAJ, JAVA, SOT) AS SELECT HYRAT_MUAJ.FITIMI AS MUAJ, HYRAT_JAVA.FITIMI AS JAVA, HYRAT_SOT.FITIMI AS SOT FROM PUBLIC.HYRAT_MUAJ INNER JOIN PUBLIC.HYRAT_JAVA ON 1=1 INNER JOIN PUBLIC.HYRAT_SOT ON 1=1 WHERE TRUE;\n" +
                    "create or replace view te_hyrat as \n" +
                        "select hyrat_muaj.fitimi as muaj, hyrat_java.fitimi as java, hyrat_sot.fitimi as sot, hyrat_muaj.fitimi_lek as muaj_lek, hyrat_java.fitimi_lek as java_lek, hyrat_sot.fitimi_lek as sot_lek \n" +
                        "from hyrat_muaj, hyrat_java, hyrat_sot\n" +
...
//                    "CREATE FORCE VIEW PUBLIC.VPROD(BARCODE, ID, EMRI, QMIMI_SHITJES, BG, FG, NJESIA, KATEGORIA_ID, SASIA, ZBRITJE) AS SELECT PRODUKTET.BARCODE, PRODUKTET.ID, PRODUKTET.EMRI, PRODUKTET.QMIMI_SHITJES, KAT_PROD.BG, KAT_PROD.FG, PRODUKTET.NJESIA, KAT_PROD.ID AS KATEGORIA_ID, PRODUKTET.SASIA, PRODUKTET.ZBRITJE FROM PUBLIC.PRODUKTET INNER JOIN PUBLIC.KAT_PROD ON 1=1 WHERE (PRODUKTET.SASIA > 0) AND (PRODUKTET.KATEGORIA_ID = KAT_PROD.ID);\n" +
                    "CREATE OR REPLACE VIEW VPROD AS \n" +
                        " SELECT PRODUKTET.BARCODE, PRODUKTET.ID, PRODUKTET.EMRI, PRODUKTET.QMIMI_SHITJES, KAT_PROD.BG, KAT_PROD.FG, PRODUKTET.NJESIA, KAT_PROD.ID \n" +
                        " AS KATEGORIA_ID, PRODUKTET.SASIA, PRODUKTET.ZBRITJE, PRODUKTET.QMIMI_SHITJES_LEK\n" +
                        " FROM PRODUKTET, kat_prod\n" +
                        " WHERE (PRODUKTET.SASIA > 0) AND (PRODUKTET.KATEGORIA_ID = KAT_PROD.ID);\n" +
                    ...
                    "create or replace view vshitjet as\n" +
                            "select konsumatoret.id as kons_id, produktet.emri as produkti, kat_prod.kategoria, produktet.qmimi_shitjes, (produktet.qmimi_shitjes - produktet.qmimi_std) as fitimi, shitjet.koha_shitjes, shitjet.cash, \n" +
                            "(shitjet.cash - produktet.qmimi_shitjes * shitjet.sasia) as kusuri, shitjet.sasia as sasia, (shitjet.sasia * (produktet.qmimi_shitjes - produktet.qmimi_std)) as totalNeto,\n" +
                            "shitjet.sasia * (produktet.qmimi_shitjes - (produktet.qmimi_shitjes * produktet.zbritje/100) + ((produktet.qmimi_shitjes - (produktet.qmimi_shitjes * produktet.zbritje/100)) * rec.tvsh/100)) as total,\n" +
                            "perdoruesi.usr as punetori, shitjet.rec_id as red_id, produktet.id as p_id, rec.tvsh, perdoruesi.pnt_id, produktet.zbritje as prod_zbritje\n" +
                            "from shitjet, konsumatoret, produktet, kat_prod, perdoruesi, rec\n" +
                            "where shitjet.prod_id = produktet.id and produktet.kategoria_id = kat_prod.id and konsumatoret.id = shitjet.kons_id and perdoruesi.id = shitjet.usr and shitjet.rec_id = rec.rec_id";
